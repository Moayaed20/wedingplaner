...
function AddonSection<T extends { id: string }, C, U extends Partial<C>>({
  ...
  renderForm: (form: C, setForm: React.Dispatch<React.SetStateAction<C>>, isEdit: boolean) => React.ReactNode;
  emptyForm: C;
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="space-y-3">{renderForm(addForm, setAddForm, false)}</div>
      ...
      {editItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="space-y-3">{renderForm(editForm, setEditForm, true)}</div>
            <div className="mt-4 flex justify-end gap-2">
...
function EditHallPage() {
  ...
  useEffect(() => {
    if (hall) {
      setFormData({
        name: hall.name, address: hall.address, city: hall.city,
  ...
  const { mutate: updateHall, isLoading: updating, error: updateError } =
    useMutation<Hall, CreateHallBody>((body, t) => HallsAPI.update(hallId, body, t));
  ...
  const { data: caterings, isLoading: loadingCat, refetch: refetchCat } =
    useApi<Catering[]>(() => CateringsAPI.list(hallId), [hallId]);
  const { data: decorations, isLoading: loadingDec, refetch: refetchDec } =
    useApi<Decoration[]>(() => DecorationsAPI.list(hallId), [hallId]);
  const { data: cars, isLoading: loadingCar, refetch: refetchCar } =
    useApi<Car[]>(() => CarsAPI.list(hallId), [hallId]);
  const { data: music, isLoading: loadingMus, refetch: refetchMus } =
    useApi<Music[]>(() => MusicAPI.list(hallId), [hallId]);
  ...
  if (loadingHall) {
    return (
      <DashboardShell navItems={navItems} userName="المشرف" userRoleLabel="جاري التحميل...">
        <p className="text-sm text-muted-foreground">جارٍ تحميل بيانات القاعة...</p>
      </DashboardShell>
    );
  }
  ...
  if (fetchError || !hall) {
    return (
      <DashboardShell navItems={navItems} userName="المشرف" userRoleLabel="خطأ">
        <p className="text-sm text-red-500">حدث خطأ أثناء تحميل البيانات: {fetchError}</p>
        <Button className="mt-4 rounded-full" onClick={() => router.push("/admin/halls")}>العودة للقائمة</Button>
      </DashboardShell>
    );
  }

  return (
    <DashboardShell navItems={navItems} userName="المشرف" userRoleLabel="تعديل القاعة">
      <div className="flex items-center gap-4 mb-6">
        <Button variant="outline" size="sm" onClick={() => router.back()} className="rounded-full">
          <ArrowRight className="h-4 w-4" />رجوع
        </Button>
        <h2 className="text-lg font-extrabold text-ink">تعديل القاعة: {hall.name}</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="space-y-1.5">
            ...
            <Input id="price_per_person" name="price_per_person" type="number" value={formData.price_per_person} onChange={handleChange} required min={0} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="min_capacity">الحد الأدنى للضيوف</Label>
            <Input id="min_capacity" name="min_capacity" type="number" value={formData.min_capacity} onChange={handleChange} required min={0} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="max_capacity">الحد الأقصى للضيوف</Label>
            <Input id="max_capacity" name="max_capacity" type="number" value={formData.max_capacity} onChange={handleChange} required min={0} />
          </div>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="space-y-1.5">
      ...
      <div className="mt-10 max-w-2xl">
        ...
        {/* Decorations */}
        {activeTab === "الديكور" && (
          <AddonSection<Decoration, CreateDecorationBody, Partial<CreateDecorationBody>>
            items={decorations ?? []}
            loading={loadingDec}
            ...
            renderRow={(d) => (
              <span className="flex items-center gap-3">
                {d.images?.[0] && (
                  <img src={d.images[0]} alt={d.theme_name} className="h-10 w-14 rounded object-cover flex-shrink-0" />
                )}
                <span>
        ...
        {activeTab === "السيارات" && (
          <AddonSection<Car, CreateCarBody, Partial<CreateCarBody>>
            ...
            renderRow={(c) => (
              <span className="flex items-center gap-3">
                {c.images?.[0] && (
                  <img src={c.images[0]} alt={c.car_name} className="h-10 w-14 rounded object-cover flex-shrink-0" />
                )}
                <span>
