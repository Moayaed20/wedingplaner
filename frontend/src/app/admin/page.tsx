...
import { LayoutGrid, Users, Building2, Sparkles, CalendarCheck, BarChart3 } from "lucide-react";
...
function AdminDashboard() {
  ...
  const { data: users, isLoading: usersLoading } = useApi<User[]>((token) => UsersAPI.list(token), []);
  ...
  return (
    <DashboardShell navItems={navItems} userName={user?.name ?? "المشرف"} userRoleLabel="نظرة عامة على المنصة">
      <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="إجمالي المستخدمين" value={String(users?.length ?? 0)} icon={Users} tone="violet" />
        <StatCard label="إجمالي القاعات" value={String(halls?.length ?? 0)} icon={Building2} tone="sky" />
        <StatCard label="إجمالي الحجوزات" value={String(bookings?.length ?? 0)} icon={CalendarCheck} tone="mint" />
        <StatCard label="إجمالي الإيرادات" value={formatSYP(totalRevenue)} icon={BarChart3} tone="peach" />
      </section>
      ...
      <section className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-[1.6fr_1fr]">
            ...
            {recentBookings.map((b) => (
              <div key={b.id} className="flex items-center gap-3">
                <div className="relative h-12 w-14 shrink-0 overflow-hidden rounded-xl">
                  <Image
                    src={b.hall?.images?.[0] ?? "https://images.unsplash.com/photo-1519741497674-611481863552?q=80&w=200&auto=format&fit=crop"}
                    alt={b.hall?.name ?? ""}
                    fill
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-ink">{b.hall?.name ?? "القاعة"}</p>
                  <p className="text-xs text-muted-foreground">{formatDate(b.event_date)}</p>
                </div>
                <BookingStatusBadge status={b.status} />
                <p className="shrink-0 text-sm font-bold text-primary">{formatSYP(b.total_price)}</p>
              </div>
            ))}
            {recentBookings.length === 0 && <p className="text-sm text-muted-foreground">لا توجد حجوزات بعد.</p>}
          </div>
        </div>
        ...
        <div className="rounded-[1.75rem] border border-border bg-white p-5 shadow-card">
          ...
          <div className="grid grid-cols-2 gap-3">
            <QuickLink href="/admin/users" label="المستخدمون" icon={Users} tone="bg-violet-100 text-violet-600" />
            <QuickLink href="/admin/halls" label="القاعات" icon={Building2} tone="bg-sky-100 text-sky-600" />
            <QuickLink href="/admin/services" label="الخدمات" icon={Sparkles} tone="bg-emerald-100 text-emerald-600" />
            <QuickLink href="/admin/bookings" label="الحجوزات" icon={CalendarCheck} tone="bg-orange-100 text-orange-600" />
          </div>
        </div>
      </section>
    </DashboardShell>
