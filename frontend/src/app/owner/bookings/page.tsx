...
const navItems = [
  { href: "/owner", label: "لوحة التحكم", icon: CalendarCheck },
...
function OwnerBookingsPage() {
  ...
  const { data: allBookings, isLoading, refetch } = useApi<Booking[]>(
    async (token) => {
  ...
  return (
    <DashboardShell navItems={navItems} userName="مالك القاعة" userRoleLabel="جميع الحجوزات">
          ...
          <tbody>
            {(allBookings ?? []).map((b) => (
              <tr key={b.id} className="border-t border-border">
                <td className="px-5 py-3 font-semibold text-ink">{b.hall?.name ?? "القاعة"}</td>
                <td className="px-5 py-3 text-ink/80">{b.customer?.name ?? "عميل"}</td>
